 ...
 * 
 * SDIO interrupt code tailored for Technexion TDM 3730 boards by
 *	Edward, Lionel & Tapani : <linuxfae@technexion.com>
 */
...
static int omap_hsmmc_wifi_device_status=0;
static struct platform_device *omap_hsmmc_wifi_platform_device = NULL;
...
static int omap_hsmmc_card_detect(struct device *dev, int slot)
{
	...
	if (mmc->version != MMC_CTRL_VERSION_2) {
		...
		//uki: active high for yuri-board
		return gpio_get_value_cansleep(mmc->slots[0].switch_pin);
	} else {
		u32 pstate = 0;
		u32 enabled = 0;
...
static int omap_hsmmc_probe(struct platform_device *pdev)
{
	...
	if (pdata->init != NULL && omap_hsmmc_wifi_platform_device == NULL) {
	...
	if (host->id == OMAP_MMC2_DEVID)
                omap_hsmmc_wifi_platform_device = pdev;

        if(pdev != NULL && omap_hsmmc_wifi_platform_device == pdev)
                omap_hsmmc_wifi_device_status = 1;
...
static int omap_hsmmc_remove(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, NULL);
        ...
        if(omap_hsmmc_wifi_platform_device == pdev)
        {
                omap_hsmmc_wifi_device_status = 0;
        }
...
extern void tncommon_wifi_device_power_switch(int onoff);
void omap_hsmmc_inform_detection(int onoff)
{
        if(omap_hsmmc_wifi_platform_device == NULL)
                return;

        if (onoff) {
		tncommon_wifi_device_power_switch(1);
                if(omap_hsmmc_wifi_device_status == 0)
                        omap_hsmmc_probe(omap_hsmmc_wifi_platform_device);
        } else {
                if(omap_hsmmc_wifi_device_status == 1)
                        omap_hsmmc_remove(omap_hsmmc_wifi_platform_device);
		tncommon_wifi_device_power_switch(0);
        }
}
EXPORT_SYMBOL(omap_hsmmc_inform_detection);
#endif
