...
static int omap_hsmmc_card_detect(struct device *dev, int slot)
{
	...
	if (mmc->version != MMC_CTRL_VERSION_2) {
		if (board_type == BOARD_TYPE_YURI) { //uki mod
...
static int __init omap_hsmmc_probe(struct platform_device *pdev)
{
	...
	printk("--------------------omap_hsmmc probe add host...\n");
...
{  /*
	...
	printk("1\n");
	memset(&parname[0],0,10);
	strcpy(&parname[0],"brd_type");
	printk("2\n");
	ret = get_option(&str,&my_param);
	printk("!!!!!!!!!!!!!!!!! OMAP_HSMMC get_option ret %d, value %d\n",ret,my_param);
	//-uki
	*/
...
static int __init oh_setup(char *str) {
    int ret = get_option(&str,&board_type);
    printk("!!!!!!!!!!!!!!!!! OMAP_HSMMC get_option ret %d, value %d\n",ret,board_type);
}
__setup("omap_hsmmc.brd_type=",oh_setup);
