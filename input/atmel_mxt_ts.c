...
#ifdef _MXT_DEBUG_
#define _PDBA(_x_, args...)	printk("_ATMEL_MXT: "_x_, args)
#define _PDB(_x_)		printk("_ATMEL_MXT: "_x_)
#else
#define _PDBA(_x_, args...)	do {} while(0)
#define _PDB(_x_)		do {} while(0)
#endif
//-uki
...
//+uki: sysfs for checking input activity
#define EVC_KOBJECT_NAME	"p347_mxt"
#define EVC_SYSFS_PARAMETER	"event_counter"

static atomic_t		event_counter;
static struct kobject*	evc_sysfs_object;

static ssize_t show_evc(struct device* dev, struct device_attributr* attr, char* buf)
{
    int val = atomic_read(&event_counter);
    atomic_set(&event_counter,0);
    //printk("EVC read %d\n",val);
    return sprintf(buf,"%d\n",val);
}

static ssize_t store_evc(struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
    long val = simple_strtol(buf,NULL,10);
    atomic_set(&event_counter,val);
    return count;
}

static struct device_attribute dev_attr_event_counter = {
    .attr = {
	.name = EVC_SYSFS_PARAMETER,
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		_PDBA("i2c_transfer ret %d for addr %d, send %d %d\n",ret,xfer[0].addr,buf[0],buf[1]);
		if (!retry) {
...
static void mxt_proc_t9_message(struct mxt_data *data, u8 *message)
{
	...
	_PDBA("mxt_proc_t9_message x=%d, y=%d\n",x,y);
	...
	_PDBA("mxt_proc_t9_message area = %d, amplitude=%d, vector=%d\n",area,amplitude,vector);
/*
	//dev_dbg(dev,
	printk(
...
*/
	fun_input_mt_slot(input_dev, id);
	...
	if (status & MXT_T9_DETECT) {
		...
		*/
			...
			input_report_abs(input_dev, ABS_PRESSURE, amplitude);
		if (status & MXT_T9_PRESS) {
			_PDB("send press\n");
                       	//singletouch workaround
...
static int mxt_read_and_process_messages(struct mxt_data *data, u8 count)
{
	...
	//_PDBA("mxt_read_and_process_messages count = %d, max_reportid=%d\n",count,data->max_reportid);
...
static irqreturn_t mxt_process_messages(struct mxt_data *data)
{
	...
	_PDBA("mxt_process_messages count=%d\n",count);
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	if (error) {
		_PDBA("failed __mxt_read_reg with %d\n",error);
		goto err_free_mem;
        }
	...
	if (error) {
	        _PDBA("failed __mxt_read_reg reset of info block with %d\n",error);
		goto err_free_mem;
        }
					...
					const char *buf, size_t count)
{
	...
	if (data->state == APPMODE) {
		_PDB("enable_irq in mxt_update_fw_store\n");
		enable_irq(data->irq);
	}
...
static void mxt_start(struct mxt_data *data)
{
	...
	printk("data->state=%d\n",data->state);
	if (data->state != APPMODE) {
		printk("enable_irq in mxt_start\n");
		enable_irq(data->irq);
		data->state = APPMODE;
	}
...
static int __devinit mxt_initialize_t9_input_device(struct mxt_data *data)
{
	...
	_PDB("mxt_initialize_t9_input_device\n");
		...
		const struct i2c_device_id *id)
{
	...
	if (!pdata) {
		printk("failed with no pdata\n");
		return -EINVAL;
	}

	data = kzalloc(sizeof(struct mxt_data), GFP_KERNEL);
	//printk("allocated data = %08p\n",data);
	...
	//error = request_irq(client->irq,mxt_interrupt,pdata->irqflags,client->name,data);
	if (error) {
...
#define PADCONF_IRQ_ADDR		0x2164
#define PADCONF_IRQ_OFFSET		0
#define PADCONF_IRQ_VALUE		0x011C
...
static int __init mxt_init(void)
{
	...
	unsigned long gpio_base = (unsigned long)ioremap(0x49054000,SZ_4K);//GPIO5 for gpio_136
	u32 reg_val;
	u32 old;
	
	printk("mxt_init\n");
	
	padconf_addr = scm_base + PADCONF_IRQ_ADDR;
	reg_val = __raw_readl(padconf_addr);
	printk("penirq padconf 0x%p ret 0x%x\n",(void*)padconf_addr,reg_val);
	if (PADCONF_IRQ_OFFSET) reg_val &= 0x0000FFFF; else reg_val &= 0xFFFF0000;
	reg_val |= (PADCONF_IRQ_VALUE << PADCONF_IRQ_OFFSET);
	__raw_writel(reg_val,padconf_addr);
	reg_val = __raw_readl(padconf_addr);
	printk("penirq padconf 0x%p ret 0x%x\n",(void*)padconf_addr,reg_val);
	/*
	old = __raw_readl(gpio_base + 0x40); //GPIO_LEVELDETECT0
	reg_val = old | (1 << 8);
	__raw_writel(reg_val,gpio_base + 0x40);
	reg_val = __raw_readl(gpio_base + 0x40);
	printk("LEVELDETECT0 for GPIO old=0x%08x, new=0x%08x\n",old,reg_val);
	*/
	
	ret = i2c_add_driver(&mxt_driver);
	printk("i2c_add_driver for atmel mxt ret %d\n",ret);
	
	
	atomic_set(&event_counter,0);
	evc_sysfs_object = kobject_create_and_add(EVC_KOBJECT_NAME,NULL);
	if (!evc_sysfs_object) {
	    printk("Can't create sysfs directory for mxt\n");
	} else {
	    ret = sysfs_create_file(evc_sysfs_object,&dev_attr_event_counter.attr);
	    printk("mxt sysfs_create_file ret %d\n",ret);
	}
	...
	printk("IRQENABLE1 for GPIO old=0x%08x, new=0x%08x\n",old,reg_val);
	*/
	return ret;
}
...
static void __exit mxt_exit(void)
{
    ...
    if (evc_sysfs_object != NULL) {
	sysfs_remove_file(evc_sysfs_object,&dev_attr_event_counter.attr);
        kobject_put(evc_sysfs_object);
        evc_sysfs_object = NULL;
    }
    i2c_del_driver(&mxt_driver);
}

module_init(mxt_init);
//device_initcall(mxt_init);
